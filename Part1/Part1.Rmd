...
$\rightarrow$ Share your screen and I will try to help
...
.UA-red[<b>
.Large[🏃]  Try it yourself and run the model ... (don't forget to load the necessary packages: `brms` & `tidyverse`)
</b>
]
]
...
.UA-red[<i><b> Does the time that people spend reading a text differs for both texts if we control for the length of the text? </i></b>]
...
But: .UA-red[WAMBS] (when to <b>W</b>orry and how to <b>A</b>void <b>M</b>isuse of <b>B</b>ayesian <b>S</b>tatistics) (Rens van de Schoot et al. (2021))
...
data_frame(x,custom,brms_default) %>%
  ...
  ggplot(aes(x = x, y = value, lty = Prior)) + 
  geom_line() + 
  scale_x_continuous("Dur_log",limits = c(1,20)) +
...
```{r, echo = FALSE}
Model4b_CJ <- readRDS(here("models", "Model4b_CJ.RDS"))
tab_model(Model4_CJ, Model4b_CJ)
```
...
Now that you're ready to set your steps in .SW-greenD[Bayesian Mixed Effects Models] there is a high probability that this will happen sometime:


.pull-left[
```{r, out.height = "99%", out.width="99%", echo = FALSE}
knitr::include_graphics("computer-says-no.jpeg")
```
]
...
- .UA-red[`brms`] is more versatile than `lme4` allowing also to estimate
...
- .UA-red[`brms`] as stepping-stone to learn `Stan`
